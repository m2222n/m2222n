<div align="center">

# 👋 Hello, I'm Taemin Jeong 🙇🏻‍♂️

**Data Engineer & AI Developer**

[![GitHub](https://img.shields.io/badge/GitHub-m2222n-181717?style=flat&logo=github)](https://github.com/m2222n)
[![Email](https://img.shields.io/badge/Email-tamin214@naver.com-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:tamin214@naver.com)

</div>

---

## 🛠️ 기술 스택

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)

### AI/ML & LLM
![OpenAI](https://img.shields.io/badge/OpenAI_GPT--4o-412991?style=for-the-badge&logo=openai&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=chainlink&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)

### Backend & Database
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL_15-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)

### Data Engineering
![Scrapy](https://img.shields.io/badge/Scrapy-60A839?style=for-the-badge&logo=scrapy&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![Airflow](https://img.shields.io/badge/Airflow-017CEE?style=for-the-badge&logo=apache-airflow&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

### Data Analysis & Visualization
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![GeoPandas](https://img.shields.io/badge/GeoPandas-139C5A?style=for-the-badge)
![Folium](https://img.shields.io/badge/Folium-77B829?style=for-the-badge)

---

## 💼 임팩트 지표

<div align="center">

| 지표 | 성과 |
|:----:|:----:|
| 📊 **데이터 수집** | **580,000+** 건 |
| 🤖 **점자 변환** | **462,000+** 건 (98.2% 정확도) |
| ⚡ **대규모 크롤링** | **78,000+** 파라미터 |
| 🗺️ **지도 시각화** | **38개** 농작물 × **2,072개** 행정동 |
| 🎯 **AI 챗봇** | **100%** 테스트 성공률 (17/17) |
| 💻 **코드 리팩터링** | **82%** 감축 (3,229 → 570줄) |

</div>

---

## 🚀 주요 프로젝트

### 1️⃣ [Crawler - AI 학습 데이터 파이프라인](https://github.com/flickdone/crawler)
> 대규모 저작권 안전 데이터 수집 및 점자 변환 시스템

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Scrapy](https://img.shields.io/badge/Scrapy-60A839?style=flat&logo=scrapy&logoColor=white)
![Airflow](https://img.shields.io/badge/Airflow-017CEE?style=flat&logo=apache-airflow&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)

**📊 성과**
- 문서 **490,127건**, 이미지 **90,697건** 수집
- 점역 **462,411건** 완료 (검증 152,398건)
- **36개** 활성 데이터 소스 (저작권 100% 클린)
- **19개** 신규 크롤러 개발 (2주)

**🔑 핵심 기술**
- Apache Airflow로 17개 DAG 자동화
- Scrapy + Playwright 동적 크롤링
- 저작권 검증 (45개 → 36개 선정)
- 무중단 서버 마이그레이션

**📅 기간**: 2025.12 ~ 2026.01 (2개월) | **역할**: 데이터 엔지니어 (3인 팀)

---

### 2️⃣ [AI_agent - LLM 기반 ETF 질의응답 챗봇](https://github.com/m2222n/AI_agent)
> RAG 파이프라인 기반 금융 정보 챗봇 (코멘토 AI 에이전트 부트캠프)

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=chainlink&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)
![FAISS](https://img.shields.io/badge/FAISS-00A3E0?style=flat)

**📊 성과**
- 테스트 성공률 **100%** (17/17 케이스)
- 질문 분류 정확도 **88.2%**
- 평균 응답 시간 **6.4초**
- Streamlit Cloud 배포 완료

**🔑 핵심 기술**
- RAG 파이프라인 (LangChain + FAISS)
- Prompt Engineering (CoT, Few-shot, 역할 기반)
- 질문 유형 자동 분류 (5가지)
- 실시간 스트리밍 응답

**📅 기간**: 2024.11 ~ 2024.12 (3주) | **역할**: AI 에이전트 개발자 (단독)

---

### 3️⃣ [ko_br_math_api - LaTeX → 한국 수학 점자 변환 API](https://github.com/flickdone/ko_br_math_api)
> 규칙 기반 수학 표기법 변환 엔진

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)

**📊 성과**
- 변환 정확도 **98.23%** (333/339)
- 코드 **82%** 감축 (3,229 → 570줄)
- **339개** 테스트 케이스 자동화
- API 안정성 **100%**

**🔑 핵심 기술**
- 26단계 BRF 변환 파이프라인
- 레거시 코드 대규모 리팩터링
- LaTeX → BRF → Unicode 점자
- FastAPI 서버 2종 구축

**📅 기간**: 2026.01 (2주) | **역할**: 백엔드 개발자 (단독)

---

### 4️⃣ [seoul-mealkit-location-analysis - 밀키트 매장 입지 분석](https://github.com/m2222n/seoul-mealkit-location-analysis)
> ML/DL 기반 최적 입지 추천 시스템

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat&logo=selenium&logoColor=white)

**📊 성과**
- 최적 모델 **SVC** (F1=0.667, Accuracy=62.2%)
- 서울시 **424개** 행정동 입지 분석
- **15개** 최적 입지 지역 도출
- 네이버 지도 리뷰 크롤링 + 12개 공공데이터

**🔑 핵심 기술**
- PCA 차원 축소
- 5-Fold Cross Validation
- 프록시 변수 설계 (매출 데이터 부재 극복)
- 피처 중요도 분석

**📅 기간**: 2023.05 (3주) | **역할**: 데이터 수집/전처리/모델링 (6인 팀, 50%)

---

### 5️⃣ [smartfarm_region_price - 농작물 재배 지역 및 가격 예측](https://github.com/m2222n/smartfarm_region_price)
> 토양 데이터 기반 최적 재배 지역 시각화 + 가격 예측 모델

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat&logo=selenium&logoColor=white)
![Folium](https://img.shields.io/badge/Folium-77B829?style=flat)
![GeoPandas](https://img.shields.io/badge/GeoPandas-139C5A?style=flat)

**📊 성과**
- 전국 **2,072개** 행정동 × **38개** 농작물 시각화
- **78,000개** 파라미터 대규모 크롤링 (7시간)
- 감귤 가격 예측 **52%** 개선, 복숭아 **49%** 개선
- [인터랙티브 지도 배포](https://m2222n.github.io/smartfarm_region_price/)

**🔑 핵심 기술**
- Selenium 대규모 배치 크롤링
- Folium 인터랙티브 지도 (그라데이션, 툴팁)
- Scikit-learn 가격 예측 (Ridge, Lasso, RF)
- 행정동↔법정동 코드 매핑

**📅 기간**: 2023.06 ~ 2023.07 (6주) | **역할**: 데이터 수집/시각화/모델링 (3인 팀, 80%)

---

## 📊 GitHub Stats

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=m2222n&show_icons=true&theme=radical&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=m2222n&layout=compact&theme=radical&hide_border=true)

</div>

---

## 🎯 핵심 역량

```
✅ LLM 기반 AI 에이전트 개발 (OpenAI GPT-4o, LangChain, RAG)
✅ 대규모 데이터 수집 파이프라인 (Scrapy, Selenium, 78K+ 파라미터)
✅ 머신러닝/딥러닝 모델링 (Scikit-learn, TensorFlow)
✅ 지리공간 데이터 분석 (GeoPandas, Folium, 인터랙티브 지도)
✅ 프롬프트 엔지니어링 (CoT, Few-shot, 88.2% 정확도)
✅ 워크플로우 자동화 (Airflow, 17개 DAG)
✅ 레거시 코드 리팩터링 (3,229줄 → 570줄, -82%)
✅ 저작권 법률 검토 (45개 소스 → 36개 선정)
```

---

<div align="center">

### 💬 Contact Me

[![Email](https://img.shields.io/badge/Email-tamin214@naver.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:tamin214@naver.com)
[![GitHub](https://img.shields.io/badge/GitHub-m2222n-181717?style=for-the-badge&logo=github)](https://github.com/m2222n)

</div>

---

<div align="center">

*"데이터로 문제를 해결하고, AI로 가치를 창출합니다"*

![](https://komarev.com/ghpvc/?username=m2222n&color=blueviolet&style=flat-square)

</div>
