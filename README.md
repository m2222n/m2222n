...
> Formlabs Form 4 프린터 4대 실시간 모니터링 + 원격 제어 웹 플랫폼 (1인 풀스택 개발) 📅 2025.11 ~ 현재
...
- Docker 컨테이너 기반 배포
- **3D 빈피킹 비전 시스템 (Phase 5)**: Open3D + FPFH + FGR + ICP 기반 30종 부품 인식 + 6DoF 자세 추정
  - 인식률 100% (easy, FGR), RMSE 1.13mm, 매칭 속도 2.25s
  - L1~L6 전체 파이프라인 구현 (영상취득 → 전처리 → DBSCAN 분할 → 인식 → 그래스프 → Modbus 로봇 통신)
  - Intel RealSense D435 캡처 모듈, Basler Blaze-112(ToF) + ace2(RGB) 연동 예정
